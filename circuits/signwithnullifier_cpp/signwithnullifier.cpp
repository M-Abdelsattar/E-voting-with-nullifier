...
void TimeLockedPuzzleGenerator_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void TimeLockedPuzzleGenerator_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
// load src
Fr_pow(&expaux[1],&circuitConstants[5],&signalValues[mySignalStart + 3]); // line circom 18
Fr_mod(&expaux[0],&expaux[1],&signalValues[mySignalStart + 2]); // line circom 18
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_pow(&expaux[1],&signalValues[mySignalStart + 5],&signalValues[mySignalStart + 7]); // line circom 20
...
// load src
Fr_mod(&expaux[2],&signalValues[mySignalStart + 4],&signalValues[mySignalStart + 1]); // line circom 22
...
mySubcomponents[aux_create+i] = aux_cmp_num;
TimeLockedPuzzleGenerator_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 11 ;
aux_cmp_num += 1;
}
}
